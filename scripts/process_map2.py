...
API_KEY = '576004cefa1b43648fd6cd7059ae8196'
#covid_json = 'lacounty_covid.json'
covid_json='community_mapped_case_data.json'
...
def process_covid():
    ...
    newaggdf=df.groupby(['Time Stamp','Region','Latitude','Longitude']).agg({'Number of cases':'sum'}).reset_index()            
    newaggdf.to_csv ('../data/Covid-19.csv', index = False, header=True)
    newdf = newaggdf.groupby(['Time Stamp','Region', 'Latitude', 'Longitude'])['Number of cases'].sum().reset_index()
...
if __name__ == "__main__":
    ...
    # newdf = pd.read_csv('../data/Covid-19.csv',header=0)
    # print('Duplicated Values')
    # dup_df=newdf[newdf.duplicated(subset=['Time Stamp','Region'])]
    # print(dup_df.to_string())
    # #newdf=newdf.groupby(['Time Stamp','Region','Latitude','Longitude']).agg({'Mumber of cases':'sum'}).reset_index()
